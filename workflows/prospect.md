...
1. Run Step 5 (Lead Discovery — user-pasted Google Maps results is the primary method, see Step 5) **first**, broadly across the target area (city + surrounding suburbs — see note below on what counts as "the area"), before Step 2, Step 2.5, or Step 4. **This means: as soon as Discovery mode and the sector+location are confirmed, immediately ask the user to paste the expanded Google Maps results (see Step 5's exact prompt).** Don't wait for the user to volunteer it, and don't run WebSearch-based discovery first "to see what turns up" — that wastes queries on a method proven weaker (see Step 5's reasoning) and delays the one method that actually finds true zero-presence businesses.
...
5. Cross-check **every remaining candidate** via WebSearch (`"[business name]" [phone number] owner Yelp BBB Facebook`) before treating any of them as confirmed — the Maps card can occasionally be stale, and this query pattern reliably confirms or refutes real website presence. **Verify the full list by default, not a sample.** Proven 2026-07-02 in Dallas (both directions — correctly found real sites for businesses wrongly assumed to have none, and correctly confirmed zero presence for genuine leads) and again 2026-09-14 in Phoenix, where a first pass checked only the top 20 of 36 candidates by review count and found a ~65% false-positive rate (13 of 20 were disqualified); going back and checking the remaining 16 surfaced 7 more genuine leads that would otherwise have been missed. If the full list is large enough that checking all of it isn't practical in one session, say so and ask the user whether to continue in a batch, rather than silently sampling — a partial check risks leaving real leads unfound and should be a visible tradeoff, not a default.
6. **Watch for the Google Maps spam/duplicate-listing pattern** while cross-checking — these are lead-gen or rank-and-rent listings, not real independent local businesses, and should be excluded even when no real website is found for them individually:
   - **Out-of-area or toll-free phone number** for a business claiming to be local (e.g. a 928 Flagstaff/Yuma number for a "Phoenix" business, an out-of-state area code, or an 888/877 toll-free number)
   - **Phone-number-prefix clustering** — a candidate's number shares an unusual prefix block with a *different*, verified-real business in the same trade (e.g. several listings all in the same `xxx-433-1xxx` block as a company that already has a real website) — almost always the same lead-gen operator running multiple listings
   - **Generic geo-keyword name** ("[City] [Trade]," "[City] [Trade] Services," "[Trade] Repair [City]") especially when paired with any other flag on this list — a real small operator almost always has an owner's name, a founder's surname, or a distinctive brand in the business name
   - **Virtual/high-rise office address** implausible for a truck-based trade business (a downtown office tower suite for an HVAC or plumbing contractor)
   - **Near-duplicate name to another candidate already flagged** (e.g. "Phoenix HVAC Contractors LLC" and "Phoenix HVAC Contractors.") — same operator, multiple listings
   - **Attribution to a lead-gen doorway domain** (a generic domain like `homehvacrepairexpert.com` rather than the business's own branded domain), or a review-count mismatch between what the doorway page claims and what the Maps card shows
   A candidate needs only one or two of these to warrant exclusion — note the specific reasoning in the do-not-duplicate list so a future session doesn't have to re-diagnose it.
